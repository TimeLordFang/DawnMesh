...
class _RoomChatSheetState extends State<RoomChatSheet> {
  ...
  Future<void> _handleSend() async {
    ...
    if (byteCount > widget.session.maxChatTextBytes) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('消息最多 ${widget.session.maxChatTextBytes} UTF-8 字节。'),
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text(s.chatRecallFailed)));
        }
      }
    }
  }
  ...
  Future<void> _handleImage() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$error'.replaceFirst('FormatException: ', '')),
          ),
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final activeMemberIds = widget.session.members
        .map((m) => m.memberId)
        .toSet();
    ...
    for (final msg in messages) {
      ...
      final rawCode =
          codeInNick ?? (msg.senderCode.isNotEmpty ? msg.senderCode : null);
      final numericCode = rawCode != null
          ? DeviceCode.toNumeric(rawCode)
          : 's_${msg.senderId}';
    ...
    final double maxAvailableHeight =
        (screenHeight - topPadding - keyboardHeight - 16).clamp(
          240.0,
          screenHeight,
        );
    final double defaultHeight = (screenHeight * (isTablet ? 0.65 : 0.74))
        .clamp(280.0, screenHeight);
