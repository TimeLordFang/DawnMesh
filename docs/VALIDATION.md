...
- Flutter analyze 通过；223 项 Flutter 测试通过，包含四位/旧六位入房、创建输入、真实 TCP 加密通信、重连异常与截止、取消竞态及认证蓝牙中继。
- Android 34 项 JVM 测试及 lint 通过；新增 Wi-Fi 状态契约与拥塞时保护控制帧的用例。
- Native C++ AddressSanitizer/UndefinedBehaviorSanitizer 检查通过；新增 16/32/48 kHz 降噪、关闭旁路、等级切换、非法帧和并发流初始化测试。
- 离线合成人声检查：16 kHz 标准/强力输出与输入按延迟对齐后的相关系数约 0.994/0.852；加合成低频与宽带噪声后强力档约 0.854。只用于排查人声被整体压没，不代表真人道路/耳机听感。
- 三种房间共用降噪选择器，设置持久化和平台失败保留原档位测试通过；360×640 与大字号布局回归通过。
- 配套 DawnMeshServer 0.2.3：`go test -race ./...`、`go vet ./...`、前端 build/typecheck、4 项前端单元测试、4/6 位密码学兼容向量、14 项浏览器回归均通过。
- 本轮没有连接真机或耳机；Wi-Fi 多机持续通话、各自戴蓝牙耳机的射频共存、29 分钟无线关闭后恢复仍需现场复测。本地 release 构建、原签名证书、versionCode 39、ZIP 16 KB 对齐和 64 位 ELF 对齐检查通过；GitHub 工作流会再独立构建发布。
