...
沿用现有房间布局、聊天、两种发言模式、耳麦入口及锁屏控件。主区域横向显示成员头像，发言和禁麦状态直接呈现在头像上；空间不足时用“更多”头像打开可滚动成员网格，不铺满 25 张卡片。
...
服务端重启不保证音频连续。SQLite 保存房间 ID、名称、角色、禁麦和成员恢复记录，以及房主最大断线时长、本次断线截止时间和空房回收截止时间。邀请码和聊天不持久化；语音密钥默认也不进入服务端。只有房主建房时主动允许管理员收听，服务器才用管理员凭证派生的 AES-GCM 密钥加密托管语音密钥，并向房内持续公布监听状态。恢复成员重新核对权限及密钥代际；未授权托管的房间若没有持钥客户端返回，不能仅凭服务器数据库恢复内容密钥。
