...
dev.11 在蓝牙 L2CAP 和 Wi-Fi TCP 客户端发现物理断链后，会按 1、2、4、8、15、30 秒退避重建链路，之后每 30 秒重试，总恢复窗口为 10 分钟。蓝牙客户端每次恢复会重新扫描房主广播，刷新可能变化的设备地址和动态 PSM，并在重新订阅数据通道前等待旧订阅完全关闭。链路恢复后会重新做邀请码 PAKE 验证并自动进房；房主在同一窗口内保留成员号和名额。Wi-Fi Direct 房主也会监测并重建意外消失的系统群组。恢复期间房间页标题下方显示进度状态，音频前台服务与锁屏控件保持运行。

蓝牙房与蓝牙耳机同时使用时，dev.11 会把相邻协议帧合并为较少的 L2CAP socket 写入，并使用 200 ms 起步、最高 400 ms 的自适应接收缓冲，降低同一蓝牙控制器同时调度耳机实时音频和房间数据造成的断续。房间底部可切换“耳机麦克风”和“手机麦克风”；选择手机麦克风后仍从蓝牙耳机播放，经典耳机可保持 A2DP 媒体输出，更适合同时播放音乐。调试日志会记录 AudioRecord、AudioTrack 和通信设备的实际路由。
