...
dev.9 在蓝牙 L2CAP 和 Wi-Fi TCP 客户端发现物理断链后，会按 1、2、4、8、15、30 秒退避重建链路，之后每 30 秒重试，总恢复窗口为 10 分钟。链路恢复后会重新做邀请码 PAKE 验证并自动进房；房主在同一窗口内保留成员号和名额。Wi-Fi Direct 房主也会监测并重建意外消失的系统群组。恢复期间房间页标题下方显示进度状态，音频前台服务与锁屏控件保持运行。
