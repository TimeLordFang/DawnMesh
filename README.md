...
当前加密 Wi-Fi 房的语音与控制都走 TCP，不使用旧版未认证的 UDP 语音端点登记。弱网丢包可能增加语音延迟。**本版房主退出后房间结束，不支持房主自动迁移**。单条加密聊天限制为 320 UTF-8 字节，保证多人昵称和历史同步的信封不超过传输帧上限。**建议三台手机都更新到 dev.7；应用层邀请码协议兼容 dev.3 至 dev.6，但新版 Wi-Fi Direct 群组凭据和旧包不兼容。dev.2 的 22 位邀请码协议也不兼容。**
...
主页和房间页右上角的终端图标可打开“调试日志”。记录默认关闭；打开后会记住开关状态，并同时收集 Dart 会话逻辑及 Android 原生音频、BLE、Wi-Fi Direct 日志。页面支持按级别筛选、关键字搜索、复制当前结果和清空，最多保留最近 400 条。

日志内容只保存在当前进程内存中，关闭记录会立即清空，重启应用也不会恢复旧内容。复制时会自动隐藏 IP、MAC 和长令牌；排障完成后建议关闭记录。旧红米语音断续复测时，可重点查看 `SunsetAudio` 的编码超时、音频缓冲和 `trackUnderruns`，以及 `SunsetBle` 的链路异常。
