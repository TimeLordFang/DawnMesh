...
> 当前测试版本：`1.0.0-beta.13`。安装包请从 [GitHub Releases](https://github.com/TimeLordFang/DawnMesh/releases) 获取。
...
房间邀请码仅保存在当前会话内存中，文字聊天退出后从本机内存清除。应用没有账号、预置公网服务或遥测上传。使用公网房时，App 只连接用户选定的自部署服务器。公网房主可在建房时选择是否允许服务器管理员收听；默认关闭，开启后监听状态会在房内持续显示。主动检查更新时会连接 GitHub。
...
打开 [GitHub Releases](https://github.com/TimeLordFang/DawnMesh/releases)，下载 `DawnMesh-1.0.0-beta.13-release.apk`。每个 Release 同时提供 SHA-256 校验文件。
