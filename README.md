...
当前加密 Wi-Fi 房的语音与控制都走 TCP，不使用旧版未认证的 UDP 语音端点登记。弱网丢包可能增加语音延迟。**本版房主退出后房间结束，不支持房主自动迁移**。单条加密聊天限制为 320 UTF-8 字节，保证多人昵称和历史同步的信封不超过传输帧上限。**建议三台手机都更新到 dev.6；应用层邀请码协议兼容 dev.3/dev.4/dev.5，但新版 Wi-Fi Direct 群组凭据和旧包不兼容。dev.2 的 22 位邀请码协议也不兼容。**
