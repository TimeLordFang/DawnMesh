  ...
  group('SessionCrypto Tests', () {
    test(
      ...
      () async {
        ...
        final message = Uint8List.fromList(utf8.encode('Hello DawnMesh'));
    ...
    test(
      'SessionCipher roundtrip encryption and decryption with AAD',
      () async {
        ...
        expect(
          ...
          plaintext.length + SessionCipher.tagBytes,
        );
    ...
    test('SessionCipher rejects replayed nonces', () async {
      ...
      expect(() async => await cipher.decrypt(packet), throwsStateError);
    });
  });

  group('SessionHandshake & SecureFrameCodec Tests', () {
    test(
      'Handshake establishes matching ciphers and seals/opens frames',
      () async {
        final host = await DeviceIdentity.generate();
        final guest = await DeviceIdentity.generate();
        const roomId = 'dawn-test-room-101';
    ...
    test('Handshake fails when remote signature is invalid', () async {
      final host = await DeviceIdentity.generate();
      final guest = await DeviceIdentity.generate();
      const roomId = 'dawn-test-room-102';
    ...
    test(
      'SecureFrameCodec rejects already sealed frame or mismatched header',
      () async {
        final keyBytes = Uint8List(32);
        final cipher = await SessionCipher.fromKey(keyBytes);
